...
**Hero's two CTAs removed.** Leonid's call, following straight from the two decisions above: "See the work" anchored to `ProjectGrid`, which renders immediately below the hero on the same page, so the click scrolled a few hundred pixels to content already in view rather than going anywhere. "Read the CV" duplicated the link `SiteFooter` had just gained. Both deleted from `hero.ts`'s `actions` array and the block in `page.tsx` that rendered them; `design-system.ts`'s `CONTRAST_PAIRS` comment for the `bg`-on-`bg-accent` pair updated to note the control that motivated it is gone, while keeping the pair listed as the bar the next filled control should clear.
...
Re-run after the hero-CTA removal: `npm run typecheck` clean, `npm run test` (96 unit tests) and `npm run test:export` (239 export tests) both pass, no test referenced `hero.actions` or either CTA's label. Checked the rebuilt `out/`: neither "See the work" nor "Read the CV" appears in `out/index.html`, and "CV (PDF)" appears in the footer of `out/index.html`, `out/about/index.html`, `out/design-system/index.html`, and `out/projects/rollhaus/index.html`.
